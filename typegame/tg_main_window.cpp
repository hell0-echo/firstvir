    ...
    , m_audioMgr(new TgAudioManager(this))
{
    ...
    connect(&m_gameTimer, &QTimer::timeout, this, &TgMainWindow::onGameTick);
    setupUI();
    loadConfigAndApply();
    setupConnections();
}
...
void TgMainWindow::initializeAppleGame()
{
    ...
    gameLayout->setContentsMargins(0, 0, 0, 0); gameLayout->setSpacing(0);

    m_appleCanvas = new TgGameCanvas(m_appleLogic, m_gameScene);
    gameLayout->addWidget(m_appleCanvas, 1);
    m_toolbar = new TgAppleBottomToolbar(m_gameScene);
    m_toolbar->setVisible(false);
    gameLayout->addWidget(m_toolbar);

    m_appleLogic = new TgAppleLogicOrchestrator(); 
...
{
    if (m_spaceLogic != nullptr) return;
    auto* layout = new QVBoxLayout(m_gameScene);
    layout->setContentsMargins(0, 0, 0, 0); layout->setSpacing(0);

    m_spaceCanvas = new TgSpaceCanvas(m_spaceLogic, m_gameScene);
    layout->addWidget(m_spaceCanvas, 1);

    m_spaceLogic = new TgSpaceGameLogic(); 
    connect(m_spaceLogic, &TgSpaceGameLogic::gameOver, this, &TgMainWindow::onSpaceGameOver);
}
...
void TgMainWindow::onGameTick()
{
    if (!m_isGameRunning) return;

    constexpr float dt = 0.016f; // ~60 FPS
    if (m_currentMode == TgGameMode::ModeApple && m_appleLogic) {
        m_appleLogic->step(dt);
        m_appleCanvas->update();
    }
    else if (m_currentMode == TgGameMode::ModeSpace && m_spaceLogic) {
        m_spaceLogic->step(dt);
        m_spaceCanvas->update();
    }
}