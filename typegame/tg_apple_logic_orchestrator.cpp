/* ------------------------------------------------------------------
//  文件名    :   tg_apple_logic_orchestrator.cpp
//  创建者    :   dev@project.cn
//  创建时间  :   2024-05-20
//  功能描述  :   拯救苹果核心调度器实现
// ----------------------------------------------------------------*/
...
TgAppleLogicOrchestrator::TgAppleLogicOrchestrator(TgThreadSync& sync, QObject* parent)
    : QObject(parent), m_sync(sync), m_pool(32)
{
}
...
void TgAppleLogicOrchestrator::updateSnapshot()
{
    ...
    snap.m_fails = m_fails;
    snap.m_isVictory = isVictory();
    snap.m_isDefeat = isDefeat();
    snap.m_entities.resize(m_activeApples.size());
    for (int i = 0; i < m_activeApples.size(); ++i) {
        const auto* a = m_activeApples[i];
        snap.m_entities[i] = { QRect(static_cast<int>(a->m_pos.x()), static_cast<int>(a->m_pos.y()), 40, 40), a->m_char, true, a->m_isBroken };
    }
    m_sync.unlockWrite();
}
